...
const RequiredHeaders = struct {
    ...
    fn init(message: dbus.ParsedMessage) !RequiredHeaders {
...
const PropertyInterfaceReq = enum {
    ...
    fn parse(message: dbus.ParsedMessage, headers: RequiredHeaders) ?PropertyInterfaceReq {
        if (message.message_type != .call) return null;
        if (!std.mem.eql(u8, headers.interface, "org.freedesktop.DBus.Properties")) return null;

        if (std.mem.eql(u8, headers.member, "Get")) {
            return .get;
        }

        if (std.mem.eql(u8, headers.member, "GetAll")) {
            return .getAll;
        }

        if (std.mem.eql(u8, headers.member, "Set")) {
            return .set;
        }
...
fn parseRequestMethod(comptime Method: type, message: dbus.ParsedMessage, required_headers: RequiredHeaders) MessageHandlerError!?Method {
    ...
    const me = std.meta.stringToEnum(MethodE, required_headers.member) orelse return null;
    switch (me) {
        inline else => |t| {
...
fn typeFieldNames(comptime T: type) []const []const u8 {
    return comptime blk: {
        const fields = std.meta.fields(T);
        var ret: [fields.len][]const u8 = undefined;
...
test typeFieldNames {
    const mpris_service = @import("mpris_service");
    const paths = typeFieldNames(mpris_service.Request);

    try std.testing.expectEqual(1, paths.len);
...
fn genIntrospectionResponseMatchingPath(comptime PathDef: type, w: *std.Io.Writer) MessageHandlerError!void {
    ...
    inline for (std.meta.fields(PathDef)) |field| {
        try w.print("{s}", .{@FieldType(PathDef, field.name).docstring});
...
fn UnionType(comptime U: type, comptime tag: std.meta.Tag(U)) type {
...
pub fn genIntrospectionResponse(comptime ServiceDef: type, path: []const u8, w: *std.Io.Writer) MessageHandlerError!void {
    ...
    if (std.meta.stringToEnum(PathEnum, path)) |pe| {
        switch (pe) {
    ...
    // Header copied from spotify introspection response
    try w.writeAll(
        \\<!DOCTYPE node PUBLIC "-//freedesktop//DTD D-BUS Object Introspection 1.0//EN"
        \\        "http://www.freedesktop.org/standards/dbus/1.0/introspect.dtd">
        \\<node>
    ...
    // I really just want an equivalent of ArrayList.initBounded w/ putBounded
    // etc. We have something in sphtud, but dependency chain is way simpler
    // with sphtud not used here
    //
    // Surely this is enough, but if not just bump it
    var alloc_buf: [4096]u8 = undefined;
    var fba = std.heap.FixedBufferAllocator.init(&alloc_buf);

    var seen_children = std.StringHashMap(void).init(fba.allocator());
...
test "genIntrospectionResponse leading paths" {
    ...
    const mpris_service = @import("mpris_service");
    {
        ...
        try genIntrospectionResponse(mpris_service, "/", &w);
...

    {
        ...
        try genIntrospectionResponse(mpris_service, "/org", &w);
...

    {
        ...
        try genIntrospectionResponse(mpris_service, "/org/mpris/", &w);
...
test "genIntrospectionResponse full definition" {
    const mpris_service = @import("mpris_service");
    const expected =
        \\<!DOCTYPE node PUBLIC "-//freedesktop//DTD D-BUS Object Introspection 1.0//EN"
        \\                  "http://www.freedesktop.org/standards/dbus/1.0/introspect.dtd">
